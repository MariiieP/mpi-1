...
//а столбцы матрицы В передаются последовательно каждому роцессу.
// Вариант 23. minj*mink(aik+bjk)
#include <stdio.h>
#include <mpi.h>
#include <time.h>
#include <stdlib.h>
...
int min_series(int *a,int *b, int n)
{
    int min = a[0]+b[0];
    for (int i=1;i<n;i++) {
        if (a[i]+b[i] < min) {
...
int main(int argc,char *argv[])
{
    ...
    int *A,*B;
    A = (int *) ( malloc(sizeof(int)*n) ); // Row
    B = (int *) ( malloc(sizeof(int)*n) ); // Col
